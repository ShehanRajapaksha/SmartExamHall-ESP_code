...
void setup() {
    ...
    webSocket.connect("ws://192.168.1.111:5000"); // should be changed with network change
}

void webSocketEvent(WebsocketsEvent event, String data) {
    if (event == WebsocketsEvent::ConnectionOpened) {
        lcd.clear();
        // lcd.print("WebSocket Connected");
...
void apireqverify(uint16_t fingerprint_id) {
    if (WiFi.status() == WL_CONNECTED) {
        ...
        if (httpResponseCode > 0) {
            ...
            StaticJsonDocument<200> responseDoc;
            deserializeJson(responseDoc, response);
            const char* pcId = responseDoc["pcId"];
            lcd.clear();
            lcd.print("Your PC:");
            lcd.setCursor(0, 1);
            lcd.print(pcId);
